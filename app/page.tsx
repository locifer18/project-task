  ...
  const fetchDashboardData = async () => {
    try {
      const [projectsRes, tasksRes] = await Promise.all([
        fetch("/api/project"),
        fetch("/api/kanban/task", { credentials: "include" }),
      ]);
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          {user?.role || "Member"} • {new Date().toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
        </p>
      </div>
          ...
          {overdueTasks.length === 0 ? (
            ...
            <div className="space-y-3">
              {overdueTasks.map((task) => (
                <div key={task.id} className="flex items-start gap-3 p-3 rounded-lg bg-red-50 dark:bg-red-500/5 border border-red-100 dark:border-red-500/10">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${
                    ...
                    task.priority === "medium" ? "bg-yellow-100 text-yellow-700 dark:bg-yellow-500/20 dark:text-yellow-300" :
